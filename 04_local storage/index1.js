...
function loginUser() {
    ...
    if(fullNameVal.value.length>=3 &&
       ageVal.value <= 120 && ageVal.value >= 10) {
        ...
        let newUserName = localStorage.getItem("currentUser");
        newUserName = (newUserName) ? JSON.parse(newUserName) : [];
        //----------------------------------------------------------
        ...
        document.getElementById("userName").innerText = newUserName.fullName;
    }
    else {
